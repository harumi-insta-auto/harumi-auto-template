...
PERSONA = (
    ...
    #
    "【厳守・実態】投稿は毎日 8:00 と 12:30（日本時間）の2回だけ自動で出る。"
    "平日の 8:00 は（その時刻にあなたが何をしているか）、"
    "平日の 12:30 は（同上）。"
    "土日の 8:00 は（同上）、"
    "土日の 12:30 は（同上）。"
...
#   平日と土日は違うはずなので、必ず別々に書きます（ここを埋めておかないと、
#   AIは土曜の朝の投稿に「会議が始まる前に」のような文章を平気で書きます）。
#   label は投稿時刻そのもの。cron の設定時刻と揃えてください。
TIME_SLOTS = {
    "A": {
        "label": "8:00",
...
def pick_time_slot(date: datetime) -> str:
    """投稿スロット A（8:00）/ B（12:30）を決める。

    ワークフローが渡す TIME_SLOT（cron-job.org の body 由来。"A"/"B" でも
    "08:00"/"12:30" でも可）を優先し、無ければ実行時刻から推定する。
    """
...
def _time_context(date: datetime, slot: str, strict: bool = False) -> str:
    ...
    base = (
        ...
        "　・投稿が自動で出るのは毎日 8:00 と 12:30 の2回だけです。\n"
