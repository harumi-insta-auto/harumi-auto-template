...
`analytics/*.jsonl` を集計し、
  1) 決定論的なデータセクション
     （アカウント推移・**A/Bテスト（2つの投稿枠の比較）**・曜日別・投稿ランキング）
...
設計上のポイントは **A/Bテスト（投稿スロット）セクションを主役に据えた**こと。
cron-job.org の2ジョブで1日2本出し、「どちらの時間帯が効くのか」に
決着をつけるのを観測の第一目的にしているため。
投稿タイプはフィード画像を前提にしているので、タイプ別集計は従属的な扱いにしてある。
...
# AIに週次の所見を書かせるときに渡す「アカウントの前提」。
#
# ★ここは書き換える場所です。
#   数字だけ渡しても、AIは的外れな一般論しか返しません
#   （「リールを増やしましょう」など、あなたがやらないことを勧めてくる）。
#   **何のために運用しているのか**を書いておくと、所見が急に自分ごとになります。
#
#   書いておくとよいこと:
#     ・どんな立場の人が、どんな頻度で投稿しているか
#     ・いま何を確かめたくて数字を見ているのか（例：投稿時間帯のA/Bテスト）
#     ・最終的な目的（フォロワー数そのものか、別の場所への導線か）
#     ・すでに分かっている事実や、やらないと決めていること
#       → これを書いておくと、済んだ話や却下した案を毎週蒸し返されずに済みます
#
#   運用しながら1行ずつ足していくのが現実的です。
ACCOUNT_CONTEXT = """- （どんな立場の人が、どんな頻度で投稿しているか。
  例：平日フル勤務の会社員が、自動投稿の仕組みで毎日2本出している）
- （いま何を確かめたいのか。例：朝と昼、どちらの時間帯が効くかのA/Bテストに決着をつけること）
...
def build_data_section(account_rows, media_rows) -> tuple[str, dict]:
    ...
    summary = {
        ...
        "account": {
            "username": (latest or {}).get("username"),
...
def build_insight_section(summary: dict) -> str:
    ...
    uname = (summary.get("account", {}) or {}).get("username") or "your_account"
    prompt = f"""あなたは Instagram アカウント @{uname} のグロース担当アナリストです。
